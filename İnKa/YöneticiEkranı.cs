...
namespace İnKa
{
    public partial class YöneticiEkranı : Form
    {
        ...
        {
            if (dataGridView1.SelectedRows.Count > 0) // Eğer bir satır seçili ise
            {
                DataGridViewRow selectedRow = dataGridView1.SelectedRows[0]; // İlk seçili satırı al

                string aliciAdi = selectedRow.Cells["aliciAdi"].Value.ToString(); // Seçilen satırdaki alıcı adını al
                string gondericiAdi = selectedRow.Cells["gondericiAdi"].Value.ToString(); // Seçilen satırdaki gönderici adını al
                string konu = selectedRow.Cells["konu"].Value.ToString(); // Seçilen satırdaki konuyu al
                string mesajBasligi = selectedRow.Cells["mesajBasligi"].Value.ToString(); // Seçilen satırdaki mesaj başlığını al

                baglantim.Open();
                OleDbCommand deletesorgu = new OleDbCommand("DELETE FROM mesajlar WHERE aliciAdi = @aliciAdi AND gondericiAdi = @gondericiAdi AND konu = @konu AND mesajBasligi = @mesajBasligi", baglantim); // Seçilen satırda yer alan veriyi silmek için bir delete sorgusu oluştur
                deletesorgu.Parameters.AddWithValue("@aliciAdi", aliciAdi); // Parametrelerle sorguya veri ekleyerek SQL Injection saldırılarını önle
                deletesorgu.Parameters.AddWithValue("@gondericiAdi", gondericiAdi);
                deletesorgu.Parameters.AddWithValue("@konu", konu);
                deletesorgu.Parameters.AddWithValue("@mesajBasligi", mesajBasligi);
                int result = deletesorgu.ExecuteNonQuery(); // Sorguyu çalıştırarak etkilenen satır sayısını result değişkenine ata

                if (result > 0) // Eğer sorgu bir veya daha fazla satır etkilediyse
                {
                    MessageBox.Show("Mesaj silindi!", "Başarılı", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    kullanicilari_goster();
                }
                else
                {
                    MessageBox.Show("Mesaj silinemedi!", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }

                baglantim.Close();
            }
            else // Eğer bir satır seçili değilse
            {
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            yeniKayit.Show();
            this.Hide();
            yeniKayit.textBox1.Text = dataGridView1.CurrentRow.Cells[2].Value.ToString();
        }

        private void button2_Click(object sender, EventArgs e)
        {
            label8.Text = KullanıcıGirişEkranı.veri2;
            try
            {
                baglantim.Open();
                OleDbCommand eklekomutu = new OleDbCommand("insert into mesajlar values('" + label8.Text + "','" + comboBox4.Text + "','" + textBox2.Text + "','" + textBox3.Text + "')", baglantim);
        ...
        private void YöneticiEkranı_Load(object sender, EventArgs e)
        {
            ...
            DataGridViewButtonColumn kblbtn=new DataGridViewButtonColumn();
            kblbtn.HeaderText = "Kabul et";
            kblbtn.Text = "Kabul";
            kblbtn.UseColumnTextForButtonValue = true; 
            kblbtn.DefaultCellStyle.BackColor= Color.Black;
            kblbtn.DefaultCellStyle.SelectionBackColor=Color.Black;
            kblbtn.Width = 70;
            dataGridView2.Columns.Add(kblbtn);

            DataGridViewButtonColumn redbtn = new DataGridViewButtonColumn();
            redbtn.HeaderText = "Reddet Butonu";
            redbtn.Text = "Reddet";
            redbtn.UseColumnTextForButtonValue = true;
            redbtn.DefaultCellStyle.BackColor = Color.Black;
            redbtn.DefaultCellStyle.SelectionBackColor = Color.Black;
            redbtn.Width = 70;
            dataGridView2.Columns.Add(redbtn);
        ...
        private void mesajlari_goster()
        {
            try
            {
                ...
                mesajlari_listele.SelectCommand.Parameters.AddWithValue("@alici", KullanıcıGirişEkranı.veri2); // Kullanıcı adını burada ekleyin
